...
description: >
  ...
  phase-by-phase with code review gates between phases. Use this skill when the user says
  ...
  features/{feature}/ folder. Also use when the user says "/implement-feature" or drags a spec
...
Orchestrate the parallel implementation of a feature specification by dispatching coder agents phase-by-phase. This skill reads a spec folder (created by `create-spec`), identifies the next phase of parallelizable work, spawns coder agents for each task, and runs a code review gate before moving to the next phase.
...
5. Track progress and commit completed phases

## Prerequisites

A `features/{feature}/` directory containing:
...
1. Read `features/{feature}/README.md`
2. Read `features/{feature}/requirements.md`
3. Parse the **Task Status** section in the README — look at the checkboxes:
   - `- [x]` = completed task (skip)
   - `- [ ]` = pending task (include)
4. Determine the **current phase**: the first phase that has any incomplete tasks
5. If all tasks in all phases are complete, report "All tasks complete!" and stop
...
For each phase starting from the current one, execute Steps 3 through 8 below, then advance to the next phase.

### Step 3: Prepare Phase Tasks

1. Read all incomplete task files for this phase from the `tasks/` subfolder
2. **Check for file overlaps**: scan the "Files to Create" and "Files to Modify" sections across all tasks in this phase. If any file appears in more than one task, warn the user:
...
For each task in the phase, spawn a coder agent using the `Agent` tool with `subagent_type: "coder"`. Spawn all agents for the phase in a **single message** so they run in parallel.
...
If any agent fails (returns an error or crashes), note the failure and continue with the remaining results. Report the failure to the user after the phase.
...
- **{requirements}**: full text of `requirements.md`
- **{task_summaries}**: for each task in this phase, the task title and the coder agent's completion summary
...
**Cap at 3 review cycles per phase.** If the third review still fails, stop and report to the user:
...
After the phase passes review (or the user chooses to proceed):
...
3. **Commit the phase**:
   ```
   git add -A
   git commit -m "feat({feature}): complete phase {N} — {brief summary}"
   ```
...
2. Spawn a code-review agent to review the full scope of changes (all files modified across all phases)
...
- **Coder agent failure**: mark the task as failed, report to user, continue with remaining tasks in the phase. The review step will catch integration issues.
...
- **Completed task summaries are brief.** One paragraph per task — not the full file contents. This keeps coder agent prompts from growing unbounded as phases progress.
- **The review gate is non-negotiable.** Every phase gets reviewed before commit. This catches integration issues early rather than letting them compound across phases.
